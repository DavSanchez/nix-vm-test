...
let
  ...
  # aarch64-linux host with a similarly-limited CPU would hit it too), so it's
  # checked against the *resolved* guest arch below, not just when the host is darwin.
  isUnsupportedOnAarch64 = imageID: lib.hasPrefix "8_" imageID;

  # Unchanged meaning: the default (same-arch) guest's fetched images (8.x filtered
  # out when that default guest is aarch64), used by tests/default.nix's skip-guard
  # and tests/rocky.nix's runTestOnEveryImage.
  defaultGuestSystem = import ../systems.nix { inherit hostSystem; };
  imagesForDefaultSystem = imagesJSON.${defaultGuestSystem} or { };
  supportedDefaultImages =
    if genericFor.${defaultGuestSystem}.guestIsAarch64
    then lib.filterAttrs (name: _: !(isUnsupportedOnAarch64 name)) imagesForDefaultSystem
    else imagesForDefaultSystem;
  images = lib.mapAttrs (k: v: fetchImage guestPkgsFor.${defaultGuestSystem} v) supportedDefaultImages;
  ...
  makeVmTestForImage = imageID: { guestSystem ? null, testScript, sharedDirs ? {}, diskSize ? null, extraPathsToRegister ? [ ], memorySize ? null, cpus ? null }:
    let
      ...
      image =
        if generic.guestIsAarch64 && isUnsupportedOnAarch64 imageID
        then throw "nix-vm-test: rocky \"${imageID}\" has no aarch64-linux image (64 KB-page-size kernel doesn't boot on Apple Silicon / similarly limited aarch64 CPUs); pass guestSystem = \"x86_64-linux\" to run this version."
        else fetchImage guestPkgs (imagesJSON.${resolvedGuestSystem}.${imageID}
          or (throw "nix-vm-test: rocky \"${imageID}\" has no image for guest system '${resolvedGuestSystem}'."));
