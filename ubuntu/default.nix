...
let
  ...
  # Unchanged meaning: the default (same-arch) guest's fetched images, used by
  # tests/default.nix's skip-guard and tests/ubuntu.nix's runTestOnEveryImage.
  ...
  makeVmTestForImage = imageID: { guestSystem ? null, testScript, sharedDirs ? {}, diskSize ? null, extraPathsToRegister ? [ ], memorySize ? null, cpus ? null }:
    let
      ...
      image = fetchImage guestPkgs (imagesJSON.${resolvedGuestSystem}.${imageID}
        or (throw "nix-vm-test: ubuntu \"${imageID}\" has no image for guest system '${resolvedGuestSystem}'."));
